...
The Marathons component SHALL render marathons in a CSS columns masonry layout with responsive column counts, using generic `hobbies-*` CSS classes shared across all Hobbies subsections.
...
Each marathon card SHALL display the image and a date label below it, using generic `hobbies-card` and `hobbies-date` CSS classes.
